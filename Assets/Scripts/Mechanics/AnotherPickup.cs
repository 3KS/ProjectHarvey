...
public class AnotherPickup : MonoBehaviour 
{
	...
	void PickUp ()
	{
		...
		if (Physics.Raycast (checkObjectInSight, out objectInSight, 1.2f))
		{
			selectedObject = objectInSight.transform;
			
			Debug.Log ("ray shot");
			
			/*if (objectInSight.collider.tag != "InspectableObject")
			{
				Debug.Log ("er no");
			...
			if (Input.GetMouseButtonDown (0) && selectedObject.tag == "InspectableObject")
			{
				Debug.Log ("clicked");
